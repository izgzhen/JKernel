...
static void expand(uint32_t new_size, heap_t *heap)
{
    ...
    while (i < new_size)
    {
        alloc_frame( get_page(heap->start_address + i, DO_MAKE, kernel_directory),
...
static uint32_t contract(uint32_t new_size, heap_t *heap)
{
    ...
    while (new_size < i)
    {
        free_frame(get_page(heap->start_address + i, !DO_MAKE, kernel_directory));
        i -= PAGE_SIZE;
    }
...
void free(void *p, heap_t *heap)
{
    ...
    if (test_footer->magic == HEAP_MAGIC && test_footer->header->is_hole == TRUE)
    {
